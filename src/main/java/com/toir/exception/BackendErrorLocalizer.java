...
public final class BackendErrorLocalizer {
    ...
    private static final Map<String, Translation> TRANSLATIONS = Map.ofEntries(
            ...
            entry("WORK_ORDER_START_REQUIRES_APPROVED_STATUS", "Only approved work orders can be started", "Начать можно только утверждённый наряд", "Faqat tasdiqlangan naryadni boshlash mumkin"),
            entry("WORK_ORDER_START_DEFECT_LIST_REQUIRED", "Approved defect list is required to start this work order", "Для запуска этого наряда требуется утверждённая ведомость дефектов", "Ushbu naryadni boshlash uchun tasdiqlangan nuqsonlar ro‘yxati kerak"),
            entry("WORK_ORDER_START_DEFECT_LIST_EQUIPMENT_MISMATCH", "Defect list belongs to a different equipment", "Ведомость дефектов относится к другому оборудованию", "Nuqsonlar ro‘yxati boshqa uskunaga tegishli"),
            entry("WORK_ORDER_START_DEFECT_LIST_NOT_APPROVED", "Defect list must be approved", "Ведомость дефектов должна быть утверждена", "Nuqsonlar ro‘yxati tasdiqlangan bo‘lishi kerak"),
            entry("WORK_ORDER_START_PERFORMER_CERTIFICATION_EXPIRED", "Assigned performer certification is expired", "Сертификация назначенного исполнителя истекла", "Tayinlangan ijrochining sertifikati muddati tugagan"),
            entry("WORK_ORDER_START_PERFORMER_SKILL_MISSING", "Assigned performer does not have required skill", "У назначенного исполнителя нет требуемой квалификации", "Tayinlangan ijrochida talab qilinadigan malaka yo‘q"),
            entry("WORK_ORDER_START_SAFETY_CHECKLIST_REQUIRED", "Safety checklist is required for shutdown/isolation work", "Для работ с остановом/изоляцией требуется чек-лист безопасности", "To‘xtatish/izolyatsiya ishlari uchun xavfsizlik cheklisti talab qilinadi"),
            entry("WORK_ORDER_START_SAFETY_CHECKLIST_BLOCKED", "Cannot start work order; critical safety checklist items are not passed", "Нельзя начать наряд: критические пункты чек-листа безопасности не пройдены", "Naryadni boshlab bo‘lmaydi: xavfsizlik cheklistidagi muhim bandlar bajarilmagan"),
            entry("WORK_ORDER_START_REPLACEMENT_FIELDS_REQUIRED", "Warehouse and replacement equipment must be set for replacement work", "Для работ по замене нужно указать склад и подменное оборудование", "Almashtirish ishida ombor va almashtirish uskunasi ko‘rsatilishi shart"),
            entry("WORK_ORDER_START_REPLACEMENT_EQUIPMENT_NOT_READY", "Replacement equipment must be AVAILABLE or RESERVED to start work order", "Для старта наряда подменное оборудование должно быть в статусе AVAILABLE или RESERVED", "Naryadni boshlash uchun almashtirish uskunasi AVAILABLE yoki RESERVED holatda bo‘lishi kerak"),
            entry("WORK_ORDER_START_REPLACEMENT_ITEM_NOT_FOUND", "Replacement equipment item is not found in selected warehouse", "Подменное оборудование не найдено на выбранном складе", "Tanlangan omborda almashtirish uskunasi topilmadi"),
