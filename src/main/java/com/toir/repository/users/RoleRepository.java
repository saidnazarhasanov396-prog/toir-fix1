...
public interface RoleRepository extends JpaRepository<Role, UUID> {
    @Query(value = "SELECT * FROM roles WHERE id = cast(:id as uuid) AND is_deleted = false LIMIT 1", nativeQuery = true)
    Optional<Role> findByIdAndIsDeletedFalse(@Param("id") UUID id);

    @Query(value = "SELECT * FROM roles WHERE is_deleted = false ORDER BY updated_at DESC", nativeQuery = true)
    List<Role> findAllByIsDeletedFalseOrderByUpdatedAtDesc();

    @Query(value = "SELECT * FROM roles WHERE id IN (:ids) AND is_deleted = false", nativeQuery = true)
    List<Role> findAllByIdInAndIsDeletedFalse(@Param("ids") Collection<UUID> ids);

    @Query(value = "SELECT EXISTS(SELECT 1 FROM roles WHERE id = cast(:id as uuid) AND is_deleted = false)", nativeQuery = true)
